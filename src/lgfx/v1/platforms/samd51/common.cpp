...
//*/
    ...
    cpp::result<uint8_t, error_t> readRegister16(int sercom_index, int addr, uint16_t reg, uint32_t freq)
    {
      // 16bitレジスタアドレスを上位バイトから送る
      uint8_t regbuf[2] = { static_cast<uint8_t>(reg >> 8), static_cast<uint8_t>(reg & 0xFF) };
      uint8_t data = 0;
      auto res = transactionWriteRead(sercom_index, addr, regbuf, 2, &data, 1, freq);
